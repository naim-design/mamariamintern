...
byId('global-month').addEventListener('change',async e=>{selectedMonth=e.target.value||currentMonthKey();await refreshAll();if(typeof refreshSeedingTodo==='function')await refreshSeedingTodo();});
...
function editSeeding(id){const x=seedingEntries.find(r=>r.id===id);if(!x)return;editingSeedingId=id;document.getElementById('seeding-date').value=x.date||'';document.getElementById('seeding-channel').value=x.channel||'Facebook Page';document.getElementById('seeding-status').value=x.status||'belum';document.getElementById('seeding-note').value=x.note||'';document.getElementById('seeding-submit-btn').textContent='Update Seeding';document.getElementById('seeding-cancel-edit').style.display='';}
...
document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{
  const sf=document.getElementById('seeding-form');if(sf&&!sf.dataset.bound){sf.onsubmit=async e=>{e.preventDefault();const date=document.getElementById('seeding-date').value,data={kind:'intern_seeding',date,monthKey:date.slice(0,7),channel:document.getElementById('seeding-channel').value,status:document.getElementById('seeding-status').value,note:document.getElementById('seeding-note').value.trim(),staffUid:currentUser?.uid||'',updatedAt:firebase.firestore.FieldValue.serverTimestamp()};if(editingSeedingId)await db.collection('entries').doc(editingSeedingId).set(data,{merge:true});else{data.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('entries').add(data)}resetSeeding();await refreshSeedingTodo();toast('Seeding disimpan.');};sf.dataset.bound='1';document.getElementById('seeding-cancel-edit').onclick=resetSeeding;}
  const tf=document.getElementById('todo-form');if(tf&&!tf.dataset.bound){tf.onsubmit=async e=>{e.preventDefault();const date=document.getElementById('todo-date').value,data={kind:'intern_todo',date,monthKey:date.slice(0,7),priority:document.getElementById('todo-priority').value,title:document.getElementById('todo-title').value.trim(),status:document.getElementById('todo-status').value,category:document.getElementById('todo-category').value,note:document.getElementById('todo-note').value.trim(),assignee:'Luqman',updatedAt:firebase.firestore.FieldValue.serverTimestamp()};if(editingTodoId)await db.collection('entries').doc(editingTodoId).set(data,{merge:true});else{data.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('entries').add(data)}resetTodo();await refreshSeedingTodo();toast('Task disimpan.');};tf.dataset.bound='1';document.getElementById('todo-cancel-edit').onclick=resetTodo;}
},400));
